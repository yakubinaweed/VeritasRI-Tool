...
##### *Analysis saved on `r format(Sys.time(), '%Y-%m-%d %H:%M:%S')`*
...
#### **Summary Table of Reference Intervals**
```{r, results='asis', echo=FALSE}
if (!is.null(params$all_results) && length(params$all_results) > 0) {
  
  # Filter for successful results
  successful_results <- Filter(function(r) r$status == "success", params$all_results)
  
  if (length(successful_results) > 0) {
    # Create a data frame from the successful results
    summary_df <- purrr::map_dfr(successful_results, function(result) {
      tibble::tibble(
        Gender = stringr::str_extract(result$label, "^\\w+"),
        `Age Range` = paste0(result$age_min, "-", result$age_max),
        `CI Lower (Lower)` = round(result$ci_low_low, 2),
        `RI Lower` = round(result$ri_low_fulldata, 2),
        `CI Lower (Upper)` = round(result$ci_low_high, 2),
        `CI Upper (Lower)` = round(result$ci_high_low, 2),
        `RI Upper` = round(result$ri_high_fulldata, 2),
        `CI Upper (Upper)` = round(result$ci_high_high, 2)
      )
    })
    ...
    cat("No successful results to display in the summary table.")
  }
} else {
  cat("No results available to generate the summary table.")
}
```
